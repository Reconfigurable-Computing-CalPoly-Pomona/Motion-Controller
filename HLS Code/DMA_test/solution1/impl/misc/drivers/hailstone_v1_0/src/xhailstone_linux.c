...
typedef struct {
    ...
    XHailstone_uio_map maps[ MAX_UIO_MAPS ];
} XHailstone_uio_info;

/***************** Variable Definitions **************************************/
static XHailstone_uio_info uio_info;
...
static int uio_info_read_name(XHailstone_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHailstone_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHailstone_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHailstone_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHailstone_Initialize(XHailstone *InstancePtr, const char* InstanceName) {
	XHailstone_uio_info *InfoPtr = &uio_info;
